@php
    $curCat = DB::table('z_product_cat')->where('id','=',$id)->first();
...
<form class="" action="{{URL::to('/updateProductCat')}}" method="post">
    {!! csrf_field() !!}
    ...
    <select id="cur" name="parent_id" class="mdb-select colorful-select dropdown-primary active ">
    ...
    <button type="button" class="btn red waves-effect" onclick="location.href='{{url('/deleteProductCat/'.Crypt::encryptString($curCat->id))}}'">Delete</button>
