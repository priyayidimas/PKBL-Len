        ...
        <div class="row">
            <div class="col-lg-4">
                <div class="card">
                    ...
                    <div class="card-body">
                        <form action="{{url('/addBlogCat')}}" method="post">
                            {!!csrf_field()!!}
                            <div class="md-form">
                            ...
                            <tbody>
                                @foreach ($cat as $ct)
                                    <tr>
                                        ...
                                        <td>{{$ct->slug}}</td>
                                        <td><a href="{{url('/blog/category/'.$ct->slug)}}" target="_blank">{{DB::table('z_trans_blog_cat')->where('category_id','=',$ct->id)->count()}}</a></td>
                                    </tr>
                                    @php
                                    ...
                                    <tr>
                                        ...
                                        <td>{{$sCat->slug}}</td>
                                        <td><a href="{{url('/blog/category/'.$sCat->slug)}}" target="_blank">{{DB::table('z_trans_blog_cat')->where('category_id','=',$sCat->id)->count()}}</a></td>
                                    </tr>
                                    @endforeach
                                @endforeach
                            </tbody>
...
<script>
    $(document).ready(function () {
        ...
        $('#modal').on('show.bs.modal', function(ev) {
            ...
            modal.find('.modal-body').load('/editBlogCat/' + id);
        });      
    });
</script>
@endsection